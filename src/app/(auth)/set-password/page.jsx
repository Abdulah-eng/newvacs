...
function SetPasswordForm() {
  ...
  // The auth/callback already exchanged the invite code for a session.
  // Supabase sets an access_token in the URL hash on the client side for invites.
  useEffect(() => {
    const supabase = createClient()

    // Handle hash-based token (Supabase invite flow sets token in hash)
    ...
    <div className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-8 shadow-2xl">
      ...
      {!sessionReady && (
        <div className="flex items-center gap-2 rounded-lg border border-amber-400/40 bg-amber-500/10 px-3 py-2.5 mb-4">
          ...
          <p className="text-[13px] text-amber-300">Verifying your invite link… please wait.</p>
        </div>
      )}
