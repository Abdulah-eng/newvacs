          ...
          <Card title="Quick documentation" icon={Stethoscope} color="13314f">
            ...
            <div className="space-y-6 max-h-[36rem] overflow-y-auto thin-scroll pr-1 pb-16">
              {[
                ...
                { id: 'reported', label: '', title: 'Patient-Reported / Self-Management', fields: ['homeBp','bpTechnique','glucoseMonitoring','weightGoals','diseaseUnderstanding','concerns','cost'].map(k => c.INTERVIEW_FIELDS?.find(f => f.key === k)).filter(Boolean) }
              ].map(g => (
                <div key={g.title}>
